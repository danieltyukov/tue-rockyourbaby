...
int heartRate = -1;                     // final heart rate value in the heartRateSensor function
...
int heartRateSensor(int start) {
  if (start == 0) {                         // if start is 0, then wait for LDRpin to become 1, then start the measurement.
    for (int x = 1; x > 0; x++) {
      if (digitalRead(LDRpin) == 1) {
        ...
        x = -10;                              // break out of the for loop after recording the time that LDRpin changed value.
      }
      delay(1);
    }
    for (int x = 1; x > 0; x++) {
      if (digitalRead(LDRpin) == 0) {            // when LDRpin changes back to 0, stop the measurement
        ...
        x = -10;                                  // break out of the for loop after recording the time that LDRpin changed back.
        }
      delay(1);
      }
  }
  ...
  heartRate = beatPerSec*60;                  // multiplying beats per second by 60 gives the beats per minute --> final return value to decision-maker  

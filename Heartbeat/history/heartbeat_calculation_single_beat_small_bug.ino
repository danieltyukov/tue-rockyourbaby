...
//    Serial.print("first time looping");
  ...
  while(second_change == 0){
    ...
    if(LDRValue != previous_value){     // Checks of the hearbeat has changed from last time
      ...
      BPM = (0.5 *60)/(0.001*timepassed); //Calculate the current bpm which is time two, because you got half a cycle, times 0.001 because the time is in millis and times 60 because it is beats per MINUTE
//      Serial.print("BPM is: ");
//      Serial.println(BPM);
      second_change = 1;
    }
